...
import { parsePagination, paginatedResponse, generateInvoiceNumber } from '../../utils/pagination';
...
export class SalesService {
  async createSale(data: CreateSaleInput, employeeId: string, stationId?: string | null) {
    // 1. Validate pump
    const pump = await prisma.pump.findFirst({
      where: { id: data.pumpId, ...(stationId && { stationId }), isActive: true },
      include: { tank: true, station: { include: { settings: true } } },
    });

    if (!pump) throw new AppError('Pump not found or inactive', HTTP_STATUS.NOT_FOUND);
    if (pump.status !== 'ACTIVE') throw new AppError(`Pump is ${pump.status}`, HTTP_STATUS.BAD_REQUEST);
    if (!pump.tank) throw new AppError('Pump has no assigned tank', HTTP_STATUS.BAD_REQUEST);

    // 2. Check tank volume
    if (pump.tank.currentLiters < data.volumeLiters) {
      throw new AppError(
        `Insufficient fuel: ${pump.tank.currentLiters.toFixed(2)}L available`,
    ...
    if (data.paymentMethod === 'FLEET') {
      if (!data.fleetAccountId) {
        throw new AppError('Fleet account required for FLEET payment', HTTP_STATUS.BAD_REQUEST);
      }
      const fleet = await prisma.fleetAccount.findFirst({
        where: { id: data.fleetAccountId, stationId: pump.stationId, isActive: true },
      });
      if (!fleet) throw new AppError('Fleet account not found', HTTP_STATUS.NOT_FOUND);

      const saleTotal = data.volumeLiters * data.pricePerLiter;
      if (fleet.currentBalance + saleTotal > fleet.creditLimit) {
        throw new AppError(
          `Fleet credit limit exceeded. Available: ${(fleet.creditLimit - fleet.currentBalance).toFixed(2)}`,
          HTTP_STATUS.BAD_REQUEST
        );
      }
    }
    ...
    const sale = await prisma.$transaction(async (tx) => {
      ...
      const newSale = await tx.sale.create({
        data: {
          ...
          totalAmount,
          paymentMethod: data.paymentMethod as 'CASH' | 'CARD' | 'FLEET' | 'MOBILE',
      ...
      if (updatedTank.currentLiters <= criticalThreshold) {
        setImmediate(() => {
          notificationService.createSystemNotification(
            ...
            `${updatedTank.name} is critically low: ${updatedTank.currentLiters.toFixed(0)}L remaining`,
            'LOW_STOCK',
            'CRITICAL'
          );
        });
        ...
        setImmediate(() => {
          notificationService.createSystemNotification(
            pump.stationId,
            '⚠️ Low Stock Alert',
            `${updatedTank.name} is running low: ${updatedTank.currentLiters.toFixed(0)}L remaining`,
            'LOW_STOCK',
            'HIGH'
          );
        });
  ...
  async getSales(query: SaleQueryInput, user: { role: string; stationId: string | null }) {
  ...
  async getSaleById(id: string, stationId?: string | null) {
    ...
    if (!sale) throw new AppError('Sale not found', HTTP_STATUS.NOT_FOUND);
    return sale;
  }

  async voidSale(id: string, data: VoidSaleInput, userId: string, stationId?: string | null) {
    const sale = await prisma.sale.findFirst({
      where: { id, ...(stationId && { stationId }), isVoided: false },
    });

    if (!sale) throw new AppError('Sale not found or already voided', HTTP_STATUS.NOT_FOUND);
